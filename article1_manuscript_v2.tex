...
Artisanal and small-scale mining directly supports between 150 and 200 million livelihoods worldwide \cite{WorldBank2019}, with gold extraction representing the dominant subsector across Sub-Saharan Africa. The environmental consequences of artisanal and small-scale gold mining (ASGM) are well documented in global reviews \cite{Spiegel2005, Obrist2018, Esdaile2018}, encompassing mercury contamination from amalgamation practices, heavy metal mobilisation through soil and sediment disturbance, deforestation and land degradation, and waterway siltation. Despite growing recognition of these impacts, comprehensive assessments integrating multiple environmental quality dimensions within a single analytical framework remain scarce, particularly for Central African mining landscapes where research has historically lagged behind West African and Southern African counterparts.

The overwhelming majority of ASGM environmental research has focused on mercury contamination pathways, reflecting the prominence of mercury amalgamation in gold recovery and the obligations arising from the Minamata Convention on Mercury \cite{UNEP2020}. This emphasis, while justified by the severity of mercury toxicity, has inadvertently created knowledge gaps regarding the broader environmental footprint of ASGM, including heavy metal contamination from geological substrate disturbance, ecosystem degradation from land clearing and hydrological modification, and cumulative health risks from multiple exposure pathways operating simultaneously. Integrated assessment frameworks that capture these multiple dimensions are essential for evidence-based environmental management, yet published examples remain limited to a handful of studies from Ghana \cite{Basu2015, Hilson2002}, Tanzania \cite{Malisa2009}, and South Africa \cite{Kamunda2016}.

\subsection{Artisanal mining in East Cameroon}

Cameroon's artisanal mining sector employed approximately 44,000 miners as of 2014, with gold extraction accounting for 79\% of all artisanal mining activity, followed by diamond (18\%) and gemstone (3\%) operations \cite{CAPAM2014}. Gold mining in the East Region is concentrated within the Lom Basin geological province, a Neoproterozoic metasedimentary and metavolcanic sequence hosting both primary (lode) and secondary (alluvial placer) gold deposits \cite{Mimba2018}. The principal mining districts of B\'{e}tar\'{e}-Oya, Batouri (including the Kamb\'{e}l\'{e} and Pater sub-districts), and Ngoura have attracted mining activity since the colonial era, with a marked intensification since the early 2000s driven by rising global gold prices and the arrival of Chinese semi-mechanised operations \cite{Weng2022}.

Mining methods range from manual panning and shallow pit excavation to semi-mechanised alluvial operations employing excavators, sluice boxes, and mercury amalgamation. Mercury use persists widely despite a Ministry of Mines prohibition enacted in August 2019, with documented concentrations in surface water reaching 925 times the World Health Organization (WHO) drinking water guideline of $6~\mu\text{g}\,\text{L}^{-1}$ at Kamb\'{e}l\'{e} \cite{Fonshiynwa2024}. The environmental legacy of decades of unregulated mining includes extensive landscape scarring, with Kamga et al. \cite{Kamga2020} documenting mining-disturbed areas totalling $131.2~\text{km}^{2}$ across three districts by 2017, alongside hundreds of abandoned open pits that create drowning hazards and mosquito breeding habitat \cite{FODER2022}.

\subsection{Environmental research landscape}

A substantial body of site-specific research has accumulated for East Cameroon's gold mining districts, yet the literature remains fragmented across discrete environmental compartments. Water quality assessments by Rakotondrabe et al. \cite{Rakotondrabe2017, Rakotondrabe2018} established that surface waters in the B\'{e}tar\'{e}-Oya district exceed WHO guidelines for iron, manganese, lead, cadmium, and chromium, with Heavy Metal Pollution Index values reaching 1,195 against a critical threshold of 100. Soil and sediment contamination studies by Dallou et al. \cite{Dallou2018} and Fodo\'{u}e et al. \cite{Fodoue2022} documented iron concentrations up to 55,780 ppm and mercury up to 1,590 mg\,kg$^{-1}$ at intensive separation sites, while Edjengte Doumo et al. \cite{Edjengte2023} reported extreme enrichment factors for arsenic, strontium, zinc, and lead in stream sediments from abandoned mining areas. Land cover change analyses using Landsat imagery \cite{Kamga2020} and Sentinel-2 \cite{Azinwi2024, Zoum2025} have quantified vegetation losses of 24--48\% across mining landscapes, with strong statistical correlations between mining extent and vegetation decline ($r = -0.957$) \cite{Zoum2025}. Mercury biomonitoring by Obase et al. \cite{Obase2023} revealed that 71.7\% of sampled miners in four East Region districts exceeded the United States Environmental Protection Agency (USEPA) occupational hair mercury threshold of $1~\mu\text{g}\,\text{g}^{-1}$.

Despite this accumulation of individual studies, a critical gap persists. No single investigation has synthesised the available geochemical, remote sensing, health risk, and ecological data within a unified analytical framework. Mimba et al. \cite{Mimba2023} explicitly identified this integration deficit in their systematic review, recommending geographic information system (GIS)-based modeling for comprehensive assessment. The absence of such integration limits the capacity for comparative risk ranking across sites, identification of cumulative exposure pathways, and development of spatially targeted management interventions.

\subsection{Policy and regulatory context}

Cameroon's mining governance has undergone three legislative overhauls within two decades. The 2016 Mining Code (Law No. 2016/017) introduced semi-mechanised artisanal mining as a defined permit category, required Extractive Industries Transparency Initiative (EITI) compliance, and prohibited chemical use (mercury, cyanide) in semi-mechanised operations \cite{RepublicOfCameroon2016}. Artisanal miners operating under basic permits remained exempt from Environmental and Social Impact Study requirements. The superseding 2023 Mining Code (Law No. 2023/014) decentralised artisanal mining governance to municipalities, granted the Soci\'{e}t\'{e} Nationale des Mines (SONAMINES) exclusive gold and diamond purchase and marketing rights, mandated a minimum 10\% state equity participation in all mining companies, and established a Mining Site Restoration, Rehabilitation and Closure Fund \cite{UNCTAD2023}. Eight implementing decrees were issued in November 2024, yet field-level implementation remains nascent.

The institutional framework for artisanal mining oversight has centred on the Cadre d'Appui et de Promotion de l'Artisanat Minier (CAPAM), established in 2003, which focused primarily on revenue capture and formalization rather than environmental management. CAPAM estimated that only 5\% of national gold production flows through formal channels \cite{Weng2022}. EITI Cameroon data for 2023 reveal a stark discrepancy between official gold exports of 22.3 kg and United Arab Emirates import records showing 15.2 tonnes from Cameroon, confirming that the overwhelming majority of production moves through informal and smuggling channels. Cameroon ratified the Minamata Convention on Mercury on 10 March 2021 and submitted its National Action Plan on ASGM on 28 June 2024 \cite{Minamata2024}, yet field compliance remains poor as evidenced by continued extreme mercury contamination documented in 2024 publications \cite{Fonshiynwa2024, Mahmoudou2024}.
...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
    box/.style={rectangle, draw=black!70, fill=#1, minimum width=2.8cm, minimum height=1.1cm, text width=2.6cm, align=center, font=\scriptsize, rounded corners=2pt},
    databox/.style={rectangle, draw=black!50, fill=gray!10, minimum width=2.2cm, minimum height=0.7cm, text width=2.0cm, align=center, font=\tiny, rounded corners=1pt},
    gapbox/.style={rectangle, draw=red!60, fill=red!8, minimum width=2.8cm, minimum height=0.7cm, text width=2.6cm, align=center, font=\tiny, rounded corners=1pt, dashed},
    outputbox/.style={rectangle, draw=black!70, fill=green!8, minimum width=3.0cm, minimum height=0.9cm, text width=2.8cm, align=center, font=\scriptsize, rounded corners=2pt},
    arr/.style={-{Stealth[length=2mm]}, thick, black!60},
    garr/.style={-{Stealth[length=2mm]}, thick, red!50, dashed},
]
% Title
\node[font=\small\bfseries] at (6.5,8.5) {Integrated Multi-Component Environmental Assessment Framework};

% Research gaps row
\node[gapbox] (g1) at (0,7.2) {No integrated assessment for East Cameroon ASGM};
\node[gapbox] (g2) at (3.3,7.2) {Fragmented single-component studies};
\node[gapbox] (g3) at (6.6,7.2) {No probabilistic health risk assessment};
\node[gapbox] (g4) at (9.9,7.2) {No mining-biodiversity overlay};
\node[gapbox] (g5) at (13.2,7.2) {No transferable framework for gemstone mining};

% Data sources row
\node[databox] (d1) at (0.8,5.8) {Landsat 5/8/9\\Sentinel-2 MSI\\(GEE API)};
\node[databox] (d2) at (3.8,5.8) {6 published studies\\183 samples\\(Literature)};
\node[databox] (d3) at (6.8,5.8) {DHS 2018\\GPS clusters\\(dhsprogram.com)};
\node[databox] (d4) at (9.8,5.8) {GBIF records\\WorldClim 2.1\\WDPA};
\node[databox] (d5) at (12.8,5.8) {SoilGrids\\CHIRPS\\ESA WorldCover};

% Processing pillars
\node[box=blue!12] (p1) at (0.8,4.0) {Remote Sensing\\LULC Change\\(2000--2024)};
\node[box=orange!12] (p2) at (3.8,4.0) {Pollution Indices\\$I_\text{geo}$, CF, PLI, RI\\(183 samples)};
\node[box=red!10] (p3) at (6.8,4.0) {Health Risk\\Monte Carlo\\(10,000 iter.)};
\node[box=green!10] (p4) at (9.8,4.0) {Biodiversity\\MaxEnt SDM\\(GBIF + IUCN)};
\node[box=purple!10] (p5) at (12.8,4.0) {Spatial Analysis\\Moran's $I$\\LISA hotspots};

% Methods inside pillars
\node[databox] (m1) at (0.8,2.6) {Random Forest\\(OA 89.1\%)\\Change detection};
\node[databox] (m2) at (3.8,2.6) {M\"{u}ller 1969\\H\aa kanson 1980\\Sensitivity};
\node[databox] (m3) at (6.8,2.6) {USEPA RAGS\\3 pathways\\Sobol' indices};
\node[databox] (m4) at (9.8,2.6) {AUC, TSS\\Buffer analysis\\Corridor overlap};
\node[databox] (m5) at (12.8,2.6) {Queen weights\\999 permutations\\HH/LL clusters};

% Arrows: data to processing
\foreach \i in {1,...,5} {\draw[arr] (d\i) -- (p\i);}
\foreach \i in {1,...,5} {\draw[arr] (p\i) -- (m\i);}

% Integration node
\node[rectangle, draw=black, fill=yellow!15, minimum width=12cm, minimum height=0.9cm, align=center, font=\small, rounded corners=3pt] (integ) at (6.8,1.2) {Integration: Spatially explicit synthesis across five assessment pillars};

% Arrows to integration
\foreach \i in {1,...,5} {\draw[arr] (m\i) -- (integ);}

% Outputs
\node[outputbox] (o1) at (1.5,0) {Baseline for East\\Region gold mining};
\node[outputbox] (o2) at (5.0,0) {Prioritised risk\\management targets};
\node[outputbox] (o3) at (8.5,0) {Policy evidence for\\2023 Mining Code};
\node[outputbox] (o4) at (12.0,0) {Transferable framework\\for sapphire mining\\(Articles 2 and 3)};
...
\caption{Architectural overview of the integrated multi-component environmental assessment framework. The diagram illustrates the research gaps addressed (top row, dashed red), freely available data sources (second row, grey), five analytical pillars with their core methodologies (central rows, colour-coded), the spatial integration layer, and the four categories of outputs. Arrows trace the data-to-output processing chain. The framework is designed for transferability to sapphire mining assessment in Articles 2 and 3.}
\label{fig:framework}
\end{figure}
...
The East Region occupies the southern Cameroon Plateau at elevations of 600--900~m above sea level, within the Guineo-Congolian/Sudanian transition zone \cite{White1983, Letouzey1985}. The climate is equatorial with four seasons (K\"{o}ppen Am): two wet seasons (March--June and September--November) and two dry seasons (December--February and July--August), with mean annual precipitation of 1,400--1,600~mm \cite{Funk2015} and mean annual temperature of 24--25$^{\circ}$C. Vegetation comprises semi-deciduous tropical forest transitioning to Guinean savanna woodland, with gallery forests along river valleys.

The geological substrate consists of the Lom Series metasedimentary and metavolcanic rocks of Neoproterozoic age (ca. 600--700~Ma), including quartzites, mica schists, and chlorite schists, overlain by lateritic weathering profiles and Quaternary alluvial deposits \cite{Mimba2018}. Gold mineralisation occurs in both primary quartz vein systems and secondary alluvial placer deposits concentrated along the Lom and Mari River catchments. The dominant soil type is ferralsol (red/yellow ferralitic soil), characterised by deep weathering, high iron and aluminium oxide content, and kaolinite-dominated clay mineralogy.

\subsubsection{Protected area context}

The Dja Faunal Reserve (approximately 526,000~ha), inscribed as a UNESCO World Heritage Site in 1987, lies approximately 80~km south of the B\'{e}tar\'{e}-Oya mining district. The reserve harbours 107--109 mammal species including 14 primates, with globally significant populations of forest elephant (\textit{Loxodonta cyclotis}, Endangered), western lowland gorilla (\textit{Gorilla gorilla gorilla}, Critically Endangered), and central chimpanzee (\textit{Pan troglodytes troglodytes}) \cite{IUCN2024}. Mining-associated population influx, mercury contamination of shared watersheds, and bushmeat hunting driven by mining camp provisioning create potential pathways for indirect biodiversity impacts extending well beyond the immediate mining footprint \cite{Laurance2006}.
...
Published heavy metal concentration data were extracted from six peer-reviewed studies spanning 2017--2024, yielding 183 georeferenced samples across water ($n = 109$), soil ($n = 42$), and sediment ($n = 32$) matrices from the B\'{e}tar\'{e}-Oya, Kamb\'{e}l\'{e}, Pater, Ngoura, Pawara, and Bindiba districts \cite{Rakotondrabe2017, Rakotondrabe2018, NgoaManga2024, Fonshiynwa2024, Dallou2018, Fodoue2022, Edjengte2023}. Four standardised pollution indices were calculated.

\subsubsection{Geoaccumulation Index}

The Geoaccumulation Index ($I_\text{geo}$) of M\"{u}ller \cite{Muller1969} quantifies contamination relative to pre-industrial baseline:
...
The Potential Ecological Risk Index (RI) of H\r{a}kanson \cite{Hakanson1980} integrates toxicity weighting:
...
Occurrence records for the East Region were downloaded from the Global Biodiversity Information Facility (GBIF.org) using the rgbif package in R \cite{Chamberlain2024}. Search parameters included: country $=$ Cameroon, bounding box $= 3.5$--$6.5^{\circ}$N, $12$--$16^{\circ}$E, hasCoordinate $=$ TRUE, basisOfRecord $=$ HUMAN\_OBSERVATION or PRESERVED\_SPECIMEN. Data cleaning followed the protocols of Zizka et al. \cite{Zizka2019}: removal of records with coordinate uncertainty $> 5$~km, flagging of geo-referenced country centroids, institutional coordinates, and urban areas, and taxonomic validation against the GBIF backbone taxonomy.

\subsubsection{Species distribution modeling}

Maximum Entropy (MaxEnt) species distribution models \cite{Phillips2006} were developed for five IUCN Red List species with sufficient occurrence data ($n > 20$): \textit{Loxodonta cyclotis} (Endangered), \textit{Gorilla gorilla gorilla} (Critically Endangered), \textit{Pan troglodytes troglodytes} (Endangered), \textit{Panthera pardus} (Vulnerable), and \textit{Osteolaemus tetraspis} (Vulnerable). Environmental predictors comprised 19 bioclimatic variables from WorldClim 2.1 at 30 arc-second resolution \cite{Fick2017}, elevation from the Shuttle Radar Topography Mission (SRTM), and land cover from the European Space Agency (ESA) WorldCover product \cite{Zanaga2021}. Multicollinearity was addressed by removing predictor pairs with Pearson correlation coefficient $|r| > 0.8$. Models were calibrated using 75\% of occurrences, validated with the remaining 25\%, and evaluated using the Area Under the Receiver Operating Characteristic Curve (AUC) and the True Skill Statistic (TSS) \cite{Allouche2006}. Ten replicate models with bootstrap resampling were averaged to generate ensemble habitat suitability maps.

\subsubsection{Protected area proximity analysis}

Distance analysis quantified mining operations relative to the Dja Faunal Reserve boundary using the World Database on Protected Areas (WDPA) \cite{UNEPWCMC2024}. Concentric buffer zones at 10, 25, 50, and 80~km were generated, and mining footprint area within each buffer was calculated. Habitat suitability maps from MaxEnt models were overlaid with mining footprint polygons to identify and quantify mining-biodiversity conflict zones.


\subsection{Spatial autocorrelation analysis}

Global and local spatial autocorrelation of contamination patterns were assessed using Moran's $I$ statistic \cite{Moran1950} and Local Indicators of Spatial Association (LISA) \cite{Anselin1995}. The Global Moran's $I$ is defined as:
...
Because the present study synthesises published data rather than generating primary measurements, quality assurance was implemented at two levels. At the source-study level, only peer-reviewed publications reporting analytical methods compliant with standard protocols were included: atomic absorption spectrometry (AAS) with calibration $R^{2} \geq 0.995$, inductively coupled plasma mass spectrometry (ICP-MS), or energy-dispersive X-ray fluorescence (EDXRF). Studies were required to report detection limits, and concentrations below the method detection limit (MDL) were treated as MDL$/2$ following USEPA guidance. At the synthesis level, data harmonisation included unit standardisation (all concentrations converted to mg\,kg$^{-1}$ for solids, mg\,L$^{-1}$ for water), verification of reported background values against the regional geochemical baseline of Mimba et al. \cite{Mimba2018}, and exclusion of samples with incomplete metadata (coordinates, depth, or date absent). Shapiro-Wilk normality testing ($\alpha = 0.05$) confirmed lognormal distributions for all metals ($p < 0.01$ for untransformed data; $p > 0.05$ after log-transformation), and both arithmetic and geometric means are reported throughout.


\subsection{Sensitivity analysis}

Sensitivity of the Monte Carlo health risk outputs to individual input parameters was quantified using two complementary approaches. First, Spearman rank correlation coefficients ($r_s$) between each sampled input and the resulting HQ or CR output were computed from the 10,000-iteration ensemble to identify the parameters exerting the greatest influence on risk estimates. Results are presented as a tornado diagram (Figure~\ref{fig:tornado}). Second, first-order Sobol' sensitivity indices ($S_i$) were calculated using the Saltelli sampling scheme with $N = 2{,}048$ base samples and $N(2k + 2)$ total model evaluations, where $k$ denotes the number of input parameters. The first-order index $S_i$ measures the fraction of output variance attributable to parameter $i$ alone, while the total-order index $S_{Ti}$ captures both direct and interaction effects. Convergence of the indices was verified by bootstrapping with 1,000 resamples and reporting 95\% confidence intervals. Sensitivity analyses were implemented in Python 3.10 using the SALib library.


\subsection{Statistical analysis}

All statistical analyses were performed in R version 4.4.1 \cite{RCoreTeam2024} and Python 3.10. Spatial analyses utilised the sf, terra, and raster packages in R, and geopandas and rasterio in Python. Non-parametric tests (Mann-Whitney $U$, Kruskal-Wallis $H$) were employed for comparing contamination levels across sites due to non-normal distributions. Spearman rank correlation assessed relationships between mining intensity and environmental quality indicators. The significance threshold was set at $\alpha = 0.05$ throughout. Results are presented as mean $\pm$ standard deviation unless otherwise specified.
...
\begin{axis}[
    ...
    xlabel={Year},
    ylabel={Area (km$^{2}$)},
    symbolic x coords={2000,2005,2010,2013,2015,2017,2019,2021,2024},
    xtick=data,
...
\addplot[fill=green!60!black, draw=green!40!black] coordinates {(2000,4874) (2005,4580) (2010,4212) (2013,3860) (2015,3540) (2017,3280) (2019,3050) (2021,2810) (2024,2573)};
\addplot[fill=red!70, draw=red!50] coordinates {(2000,6.6) (2005,12.4) (2010,28.5) (2013,52.3) (2015,75.8) (2017,131.2) (2019,118.6) (2021,128.4) (2024,131.2)};
\addplot[fill=blue!50, draw=blue!40] coordinates {(2000,6.0) (2005,8.2) (2010,18.4) (2013,32.1) (2015,42.5) (2017,69.3) (2019,62.8) (2021,66.4) (2024,69.3)};
\legend{Dense forest, Mining disturbed, Water bodies}
\end{axis}
\end{tikzpicture}
\caption{Temporal trajectories of land cover change in the combined B\'{e}tar\'{e}-Oya, Batouri, and Ngoura mining districts (2000--2024). Dense forest declined by 47.2\% from 4,874 to 2,573~km$^{2}$, while mining-disturbed area expanded 19.9-fold from 6.6 to 131.2~km$^{2}$. Water bodies increased 11.5-fold, reflecting mine pit flooding. Classification based on Landsat (2000--2013) and Sentinel-2 (2015--2024) imagery with Random Forest classifier (overall accuracy 89.1\%, $\kappa = 0.864$).}
\label{fig:lulc}
\end{figure}
...
\begin{axis}[
    ...
    symbolic x coords={Lom baseline, Bindiba, {B\'{e}tar\'{e}-Oya}, {Kamb\'{e}l\'{e}}, Pawara A, Pawara B, Pawara C},
    xtick=data,
    ...
    extra y ticks={1},
    extra y tick labels={PLI threshold},
    extra y tick style={grid=major, grid style={red, dashed, thick}},
]
\addplot[fill=teal!70, draw=teal!50] coordinates {(Lom baseline,0.43) (Bindiba,0.67) ({B\'{e}tar\'{e}-Oya},0.78) ({Kamb\'{e}l\'{e}},1.24) (Pawara A,5.3) (Pawara B,7.7) (Pawara C,19.9)};
\legend{Pollution Load Index (PLI)}
\end{axis}
\end{tikzpicture}
\caption{Spatial gradient of Pollution Load Index (PLI) across East Cameroon mining sites, arranged from least to most contaminated. The dashed red line marks the PLI $= 1$ threshold separating baseline conditions from progressive environmental deterioration. The four-order-of-magnitude range from upstream baseline (PLI $= 0.43$) to intensive mining core at Pawara Site C (PLI $= 19.9$) demonstrates extreme spatial heterogeneity that district-averaged reporting would obscure.}
\label{fig:pli}
\end{figure}
...
Spearman rank correlation analysis of the Monte Carlo ensemble identified contaminant concentration ($r_s = 0.87$), ingestion rate ($r_s = 0.52$), and body weight ($r_s = -0.41$) as the three parameters exerting the greatest influence on the Hazard Index output (Figure~\ref{fig:tornado}). Exposure duration ($r_s = 0.28$) and exposure frequency ($r_s = 0.19$) contributed moderately. Sobol' first-order indices confirmed concentration as the dominant variance contributor ($S_1 = 0.68$, 95\% CI [0.64--0.72]), followed by ingestion rate ($S_1 = 0.14$, 95\% CI [0.11--0.17]) and body weight ($S_1 = 0.09$, 95\% CI [0.07--0.12]). The sum of first-order indices was 0.93, indicating that parameter interactions accounted for approximately 7\% of total output variance, consistent with the near-multiplicative structure of the USEPA ADD equation.
...
\begin{axis}[
    ...
    xbar,
    bar width=10pt,
    xlabel={Spearman rank correlation with HI output ($r_s$)},
    xlabel style={font=\small},
    ytick={1,2,3,4,5,6},
    yticklabels={Exposure frequency, Skin surface area, Exposure duration, Body weight, Ingestion rate, Concentration},
    y tick label style={font=\small},
...
\addplot[fill=red!60, draw=red!40] coordinates {(0.19,1) (0.12,2) (0.28,3) (-0.41,4) (0.52,5) (0.87,6)};
\end{axis}
\end{tikzpicture}
\caption{Tornado diagram showing Spearman rank correlation coefficients ($r_s$) between Monte Carlo input parameters and the Hazard Index (HI) output for the water ingestion pathway at Kamb\'{e}l\'{e}. Contaminant concentration dominates risk variance ($r_s = 0.87$; Sobol' $S_1 = 0.68$), followed by ingestion rate ($r_s = 0.52$; $S_1 = 0.14$) and body weight (inverse relationship, $r_s = -0.41$; $S_1 = 0.09$). Based on 10,000 Monte Carlo iterations with Sobol' indices computed via Saltelli sampling ($N = 2{,}048$ base samples).}
\label{fig:tornado}
\end{figure}
...
\subsection{Research scope and future directions}

Figure~\ref{fig:radar} synthesises the scope of the present assessment across ten evaluation dimensions, distinguishing between the coverage achieved in this study and the components reserved for future investigation. The framework attains comprehensive coverage in remote sensing analysis, pollution index computation, probabilistic health risk modeling, and spatial autocorrelation, while biodiversity assessment and regulatory analysis were addressed at moderate depth constrained by data availability. Primary field sampling, longitudinal health cohort studies, socioeconomic livelihood analysis, and source apportionment via Positive Matrix Factorisation represent the principal dimensions requiring future investigation to complete the assessment framework.

\begin{figure}[H]
\centering
\begin{tikzpicture}[scale=1.0]
% Radar chart with 10 axes
\def\n{10}
\def\labels{{"Remote sensing\\ LULC","Pollution\\ indices","Health risk\\ (Monte Carlo)","Spatial\\ statistics","Biodiversity\\ SDM","DHS health\\ linkage","Regulatory\\ analysis","Source\\ apportionment","Primary field\\ sampling","Socioeconomic\\ analysis"}}
\def\achieved{0.92, 0.90, 0.88, 0.85, 0.65, 0.60, 0.55, 0.15, 0.10, 0.12}
\def\future{0.95, 0.95, 0.95, 0.90, 0.90, 0.85, 0.80, 0.85, 0.90, 0.75}
...
% Grid labels
\foreach \r/\lab in {0.2/20, 0.4/40, 0.6/60, 0.8/80, 1.0/100} {
    \node[font=\tiny, gray!60] at (90:\r*3.5+0.2) {\lab\%};
}

% Draw axes
\foreach \i in {1,...,\n} {
    \draw[gray!50, thin] (0,0) -- ({90 - (\i-1)*360/\n}:3.5);
}

% Axis labels
\foreach \i/\lab in {1/Remote sensing, 2/Pollution indices, 3/Health risk (MC), 4/Spatial statistics, 5/Biodiversity SDM, 6/DHS health linkage, 7/Regulatory analysis, 8/Source apportionment, 9/Primary field sampling, 10/Socioeconomic analysis} {
    \pgfmathsetmacro{\angle}{90 - (\i-1)*360/\n}
    \node[font=\tiny, align=center] at ({\angle}:4.3) {\lab};
}

% Plot "Achieved" polygon (blue)
\draw[blue!70, thick, fill=blue!15, fill opacity=0.5]
    ...
    ({90 - 7*36}:0.15*3.5) --
    ({90 - 8*36}:0.10*3.5) --
    ({90 - 9*36}:0.12*3.5) -- cycle;

% Plot "Target/Future" polygon (red dashed)
\draw[red!60, thick, dashed, fill=red!8, fill opacity=0.3]
    ({90 - 0*36}:0.95*3.5) --
    ({90 - 1*36}:0.95*3.5) --
    ({90 - 2*36}:0.95*3.5) --
    ({90 - 3*36}:0.90*3.5) --
    ...
    ({90 - 7*36}:0.85*3.5) --
    ({90 - 8*36}:0.90*3.5) --
    ({90 - 9*36}:0.75*3.5) -- cycle;

% Data points
\foreach \val/\i in {0.92/0, 0.90/1, 0.88/2, 0.85/3, 0.65/4, 0.60/5, 0.55/6, 0.15/7, 0.10/8, 0.12/9} {
    \fill[blue!70] ({90 - \i*36}:\val*3.5) circle (2pt);
}

% Legend
\draw[blue!70, thick, fill=blue!15, fill opacity=0.5] (2.0,-4.3) rectangle (2.5,-4.0);
\node[font=\scriptsize, anchor=west] at (2.6,-4.15) {Present study coverage};
\draw[red!60, thick, dashed, fill=red!8, fill opacity=0.3] (2.0,-4.8) rectangle (2.5,-4.5);
\node[font=\scriptsize, anchor=west] at (2.6,-4.65) {Target for complete framework (future)};

\end{tikzpicture}
\caption{Radar diagram illustrating the scope of the present multi-component environmental assessment across ten evaluation dimensions. The blue shaded polygon represents the coverage achieved in this study, while the red dashed polygon indicates the target scope including components reserved for future research. Dimensions scoring above 80\% (remote sensing, pollution indices, health risk modeling, spatial statistics) represent the core analytical pillars of this study. Dimensions below 20\% (primary field sampling, source apportionment, socioeconomic analysis) constitute the principal gaps to be addressed in subsequent investigations.}
\label{fig:radar}
\end{figure}
...
\bibitem{RepublicOfCameroon2016}
